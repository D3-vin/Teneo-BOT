...
def sign_siwe_for_form(private_key: str) -> Dict[str, str]:
    """Creates message in format expected by deform for Twitter:
...
def sign_siwe_for_discord_form(private_key: str) -> Dict[str, str]:
    """Creates message in format expected by deform for Discord:
    - No leading spaces in lines
    - Address in EIP-55 format (checksummed), same in message and address/ethAddress fields
    - Timestamp with milliseconds
    - Uses Discord-specific form ID
    """
    account = Account.from_key(private_key)
    wallet = account.address  # checksummed
    # Generate nonce only from [A-Za-z0-9], as in sniffer examples
    ...
    message = (
        "extra-points.teneo.pro wants you to sign in with your Ethereum account:\n"
        f"{wallet}\n\n"
        "Sign this message to verify ownership of your wallet address. This request will not trigger a blockchain transaction or cost any gas fees. Form ID: 21093049-ba5e-45d2-981b-cb23479de927\n\n"
        "URI: https://extra-points.teneo.pro\n"
        "Version: 1\n"
        "Chain ID: 1\n"
        f"Nonce: {nonce}\n"
